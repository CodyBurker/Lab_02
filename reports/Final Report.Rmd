...
Our base model will look at the relationship of the natural log of each state's cases on May 31st, 2020 per 100k residents with the natural log of state's population density. 
...
We first reviewed the histograms of our new input variables, as shown in Figure \@ref(fig:model2variables). Aside from the spike at zero (which indicates states that did not wish to implement this specific policy), the variables look fairly normal. Therefor our team did not opt to implement any type of variable transformation. 
...
And finally for our most complex model, we will account for the number of days each state had:

* Restaurant Shutdowns
* Bar Shutdowns
* Gym Shutdowns 

We have also included a variable to account for the speed each state declared a State of Emergency. This variable is the number of days between the declared State of Emergency and the day the WHO declared Covid-19 a pandemic (3/11/2020).
