...
Residuals_Plot2 = ggplot(data = Model_Data, aes(x = model2_prd, y = model2_res)) + 
  geom_point() + stat_smooth(se = TRUE)+
  ylab('Medium Model Residuals')+xlab('Predictors')
...
```{r homoskedastic_medium, echo=FALSE, message=FALSE,  fig.cap="Homoskedastic Plots for Medium Complex Model", warning=FALSE, fig.height=3}
...
The Residual/Input plot for the log of the population density still displays a flaring of the standard error of the resiudals at the extremes, as shown in Figure 7. The business mask mandate and interstate travel quarantine also display inconsistent standard errors. These factors suggest the potential for heteroskedastic errors. To mitigate this, we will use robust standard error when reviewing the significance level of medium complexity model's coefficients.
