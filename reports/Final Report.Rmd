...
5. State of Emergency declared in relationship to 3-11-2020 (Date that the World Health Organization declared the Covid-19 Pandemic) relationship to 3-11-2020 (Date that the World Health Organization declared the Covid-19 Pandemic)
...
In our final model we choose to include data that more likely supported the information covered in our medium complexity model. For example, the shutting down of restaurants, bar, and gyms made it much easier for the public to comply with the stay at home mandate. If the businesses of the state are closed, where could people congregate? Unfortunately, this may lead to colinearily between the variables. This colinearily may add noise to our model if shutdowns occurred simultaneously or alternatively if shutdowns occurred at the same time as the stay at home order. We expect relationship between long closures and lower infection rates. Finally we attempted to add a variable to capture how quickly a state responded to this crisis. Preemptive policies will lead to less initial cases and therefore initial vectors to spread the disease. This variable is the days between when a state declared a state of emergency in relation to the day the World Health Organization (WHO) declared Covid-19 a pandemic. We expect negative and small numbers (States of Emergency called before or around 3/11/2020) to align with curtailed rates of infection.



We first reviewed the histograms of our new input variables. The speed of each state's State of Emergency declaration closely aligns to a normal distribution. However each of the shutdown variables displays a slight skew. While not ideal, this skew does not span over factors of tens and therefore we did not feel it appropriate to use a log transformation. 
...
Homoskedastic_Plot
```
The Residual/Input plot displays a flaring of the standard error at the extremes which implies the potential for Homoskedastic errors. The log transformation of each state's population density and has the same effect in any model built off the base model, therefore we will use robust standard error when reviewing the significance level of all model's coefficients.

# Normal Residuals

Lastly we have a plot of each model's residuals to ensure they are normal. As you can see from the histograms below, the residual's are in fact normally distributed fro all models. 
