...
input_4<-ggplot(Model_Data, aes(x=travel.total)) + 
   geom_histogram(color="black", fill="grey")+
   ylab('Frequency')+xlab(paste('Interstate Travel Quarantine','\n','(Days in Place)'))
...
From the Residual/Predicted Values plot shown in Figure \@ref(fig:residualsplot), one can see that the base model's predicted values have a fairly linear relationship with residuals implying that a Linear Conditional Expectation exists for all models.

## Perfect Collinearity

Our base model cannot possibly have perfect collinearity because it only has one input variable. We evaluated multicollinearity using variance inflation factors for our medium complexity model and complex model. Our medium complexity model's variance inflation factors are particularly small (less than 2) and imply that there is no concern for multicollinearity. However our complex model's variance inflation factors are quite large (between 5 and 9 for several of our variables). This implies that there may be multicollinearity, though the judgment of its true existence is subjective. If after reviewing the model coefficients we believe that this potential multicollinearity creates confusion in the model, we could limit our input variables to one variable that we believe is representative of all the shut down inputs (for example, the model could include only the restaurant shutdowns instead of restaurant, bar, and gym shutdowns).

## Homoskedastic Error

To evaluate the presence of heteroskedastic errors, we plotted our model's residuals against its independent variables. For our base model, we plotted the base model residuals against the log transformation of each state's population density. This is shown in Figure \@ref(fig:homoskedastic).

```{r homoskedastic, echo=FALSE, message=FALSE, warning=FALSE, fig.height=3, fig.cap="Homoskedasticity Plot for Base Model"}
...
The Residuals/Input plot displays a flaring of the standard error at the extremes which implies the potential for heteroskedastic errors.

For our medium complexity model, we plotted the residuals against the model's four dependent variables:
...
```{r hokmoskedasticmedium, echo=FALSE, fig.cap="Homoskedasticity Plots for Medium Complex Model", fig.height=3, message=FALSE, warning=FALSE}
...
The Residuals/Input plot for the log of the population density again displays a flaring of the standard error of the residuals at the extremes, as shown in Figure \@ref(fig:hokmoskedasticmedium). The business mask mandate and interstate travel quarantine also display inconsistent standard errors. These factors suggest the potential for heteroskedastic errors. To mitigate this, we will use robust standard error when reviewing the significance level of the medium complexity model's coefficients.

The log transformation of each state's population density also affects the complex model (suggesting the existence of heteroskedastic errors within the complex model), therefore we will use robust standard error when reviewing the significance level of all model's coefficients.

## Normal Residuals

To assess our models against the last CLM assumption, we have histograms of each model's residuals to ensure they are normal. As you can see from Figure \@ref(fig:residshist), the residuals are in fact normally distributed for all models.
...
Additionally, our variables capture the length of time a mandate was in place, but not how effectively the mandate was enforced. For example, it is possible a state implemented an interstate travel quarantine but did not have the required infrastructure to enforce it. This means that we may not be capturing the full effects of an interstate travel quarantine, but instead the effect of a state declaring an interstate travel quarantine.
